...
#include <core.p4>
#include <tna.p4>
#include "parser.p4"    //4     exact headers and initial metadata_t
#include "headers.p4"   //3     metadata_t and header_t
#include "processor.p4" //6
#include "types.p4"     //2
#include "config.p4"    //1
#include "fragcheck.p4" //5
...
control Ingress(
    ...
    action ipv4_forward(mac_addr_t dst_addr_m, egress_spec_t port) {
    ...
    //Register<type of data ,type of index>(bit<32> size, initial_value=0);
    ...
    action add_count_action(){
		// ig_md.count加1，若达到hdr.ngaa.count（degree）则清零
    ...
    apply {
        if(hdr.ngaa.isValid()) {
            ...
            if(ig_md.is_aggregation == 1 ){
				...
				// 如果hdr包是ack，那么重置ig_md.frag_id 为 hdr.ngaa.frag_id
				// 如果不是，那么ig_md.frag_id 为 ig_md.index
